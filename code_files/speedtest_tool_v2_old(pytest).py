from ssh_network_conf import ssh_network_conf
...
def alpha_to_numeric(r):
    ...
    start_row = int(re.search("\d+", r["start"]).group(0))
    end_row = int(re.search("\d+", r["end"]).group(0))
    return {
        "start_row": start_row,
        "start_col": start_col,
        "end_row": end_row,
...
def trigger_test(network_conf):
    ...
    template_path = "/opt/n1/v4/template.xlsx"
